...
func TestTracesPusher_partitioningByResourceAttributes(t *testing.T) {
	// Build input with 4 distinct resources, each with 1 scope + 1 span:
	//  - two resources share service.name "svc-a" but differ in service.instance.id
	//  - one resource has service.name "svc-b"
	//  - one resource has no service.name at all
	input := ptrace.NewTraces()

	rs1 := input.ResourceSpans().AppendEmpty()
	rs1.Resource().Attributes().PutStr("service.name", "svc-a")
	rs1.Resource().Attributes().PutStr("service.instance.id", "inst-1")
	span1 := rs1.ScopeSpans().AppendEmpty().Spans().AppendEmpty()
	span1.SetName("span from svc-a inst-1")
	span1.SetTraceID(pcommon.TraceID{1})

	rs2 := input.ResourceSpans().AppendEmpty()
	rs2.Resource().Attributes().PutStr("service.name", "svc-a")
	rs2.Resource().Attributes().PutStr("service.instance.id", "inst-2")
	span2 := rs2.ScopeSpans().AppendEmpty().Spans().AppendEmpty()
	span2.SetName("span from svc-a inst-2")
	span2.SetTraceID(pcommon.TraceID{2})

	rs3 := input.ResourceSpans().AppendEmpty()
	rs3.Resource().Attributes().PutStr("service.name", "svc-b")
	span3 := rs3.ScopeSpans().AppendEmpty().Spans().AppendEmpty()
	span3.SetName("span from svc-b")
	span3.SetTraceID(pcommon.TraceID{3})

	rs4 := input.ResourceSpans().AppendEmpty()
	rs4.Resource().Attributes().PutStr("host.name", "host-1")
	span4 := rs4.ScopeSpans().AppendEmpty().Spans().AppendEmpty()
	span4.SetName("span without service.name")
	span4.SetTraceID(pcommon.TraceID{4})

	expectedKey := func(attrs map[string]string) []byte {
		m := pcommon.NewMap()
		for k, v := range attrs {
			m.PutStr(k, v)
		}
	...
	t.Run("resource_attributes_partitioning", func(t *testing.T) {
		config := createDefaultConfig().(*Config)
		...
		// 4 resources -> 4 messages.
		records := fetchKgoRecords(t, fakeCluster.ListenAddrs(), config.Traces.Topic, 4)
		require.Len(t, records, 4, "expected 4 messages (one per resource)")

		svcAKey := expectedKey(map[string]string{"service.name": "svc-a"})
		svcBKey := expectedKey(map[string]string{"service.name": "svc-b"})

		// Both svc-a resources must share a key despite differing
		// service.instance.id values, svc-b must get a different key, and the
		// resource without service.name must have a nil key.
		var keys [][]byte
		...
		require.ElementsMatch(t, [][]byte{svcAKey, svcAKey, svcBKey, nil}, keys)

		// Combine deserialized traces for content comparison.
	...
	t.Run("multiple_resource_attributes", func(t *testing.T) {
		config := createDefaultConfig().(*Config)
		config.PartitionTracesByResourceAttributes = []string{"service.name", "service.instance.id"}
		exp, fakeCluster := newKgoMockTracesExporter(t, *config,
			componenttest.NewNopHost(), config.Traces.Topic)
		defer fakeCluster.Close()
		...
		// With service.instance.id included, the two svc-a resources must no
		// longer share a key. The svc-b resource omits service.instance.id, so
		// its key hashes service.name only.
		require.ElementsMatch(t, [][]byte{
			expectedKey(map[string]string{"service.name": "svc-a", "service.instance.id": "inst-1"}),
			expectedKey(map[string]string{"service.name": "svc-a", "service.instance.id": "inst-2"}),
			expectedKey(map[string]string{"service.name": "svc-b"}),
			nil,
		}, func() (keys [][]byte) {
			for _, record := range records {
				keys = append(keys, record.Key)
	...
	t.Run("jaeger_encoding_disables_resource_attributes_partitioning", func(t *testing.T) {
		config := createDefaultConfig().(*Config)
		config.Traces.Encoding = "jaeger_json"
		config.PartitionTracesByResourceAttributes = []string{"service.name"}
		exp, fakeCluster := newKgoMockTracesExporter(t, *config,
		...
		// Jaeger encodings produce one message per span keyed by trace ID,
		// regardless of partition_traces_by_resource_attributes.
		records := fetchKgoRecords(t, fakeCluster.ListenAddrs(), config.Traces.Topic, 4)
		require.Len(t, records, 4, "expected 4 messages (one per span) for Jaeger encoding")
		var keys [][]byte
		for _, record := range records {
		...
		require.ElementsMatch(t, [][]byte{
			[]byte(pcommon.TraceID{1}.String()),
			[]byte(pcommon.TraceID{2}.String()),
			[]byte(pcommon.TraceID{3}.String()),
			[]byte(pcommon.TraceID{4}.String()),
		}, keys, "keys must be trace IDs, not resource attribute hashes")
	})
}
