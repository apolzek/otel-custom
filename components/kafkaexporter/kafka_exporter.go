...
func newTracesExporter(config Config, set exporter.Settings) *kafkaExporter[ptrace.Traces] {
	...
	// and resource attribute partitioning when they are configured.
	switch config.Traces.Encoding {
	case "jaeger_proto", "jaeger_json":
		config.PartitionTracesByID = false
		config.PartitionTracesByResourceAttributes = nil
	}
...
// resourceAttributesHashKey returns a partition key derived from the values of
// the given resource attribute keys, or nil if attributeKeys is empty or none
// of the attributes are present in the resource.
func resourceAttributesHashKey(res pcommon.Resource, attributeKeys []string) []byte {
	if len(attributeKeys) == 0 {
		return nil
	}
	keyAttrs := pcommon.NewMap()
	attrs := res.Attributes()
	for _, attrKey := range attributeKeys {
		if v, ok := attrs.Get(attrKey); ok {
			v.CopyTo(keyAttrs.PutEmpty(attrKey))
		}
	}
	if keyAttrs.Len() == 0 {
		return nil
	}
	hash := pdatautil.MapHash(keyAttrs)
	return hash[:]
}
