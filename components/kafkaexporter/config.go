...
type Config struct {
	...
	// PartitionTracesByResourceAttributes is the list of resource attribute keys used to
	// partition trace messages. When non-empty, spans are grouped by resource, and the
	// message key of outgoing trace messages is set to a hash of the values of the
	// configured resource attributes. All spans whose resource has the same values for
	// these attributes (e.g. ["service.name"]) are routed to the same partition.
	//
	// Resource attributes absent from a resource are ignored when computing its key;
	// if none of the configured attributes are present, the message key is left unset
	// and partition selection falls back to the Kafka client's default strategy.
	//
	// Mutually exclusive with partition_traces_by_id and
	// traces::message_key_from_metadata_key.
	//
	// NOTE: this does not have any effect for Jaeger encodings. Jaeger encodings always
	// use the trace ID for the message key.
...
func (c *Config) Validate() error {
	...
	if c.Traces.MessageKeyFromMetadataKey != "" &&
		(c.PartitionTracesByID || len(c.PartitionTracesByResourceAttributes) > 0) {
